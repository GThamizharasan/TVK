        ...
        >
            ...
            >
              {points.map((point, i) => (
                <div 
                  ...
                  style={{ width: itemsPerView === 1 ? '100%' : 'calc(50% - 12px)' }}
                >
                  <div className={`bg-white dark:bg-gray-800 p-8 md:p-10 rounded-[3rem] border border-gray-100 dark:border-gray-700 shadow-lg h-full flex flex-col justify-between group relative overflow-hidden transition-all duration-300 ${i === current || (itemsPerView === 2 && i === current + 1) ? 'opacity-100 scale-100 shadow-2xl' : 'opacity-40 scale-95 blur-[1px]'}`}>
                    ...
                    <div className="relative z-10">
                      <div className="w-14 h-14 bg-red-600 text-white rounded-2xl flex items-center justify-center font-black text-2xl shadow-lg shadow-red-600/30 mb-8">
                        {String(i + 1).padStart(2, '0')}
                      </div>
                      <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white mb-4 uppercase tracking-tight group-hover:text-red-600 transition-colors">
                        {point.title}
                      </h3>
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-50 dark:border-gray-700/50">
                      <button 
                        onClick={onExplore}
                        className="text-[10px] font-black uppercase tracking-widest text-red-600 dark:text-red-400 flex items-center gap-2 group/btn"
                      >
                        Learn More 
          ...
          <div className="flex justify-between items-center mt-12">
              ...
              {points.slice(0, points.length - (itemsPerView - 1)).map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    i === current ? 'w-8 bg-red-600' : 'w-2 bg-gray-300 dark:bg-gray-700 hover:bg-red-300'
                  }`}
                  aria-label={`Go to vision point ${i + 1}`}
                />
              ))}
            </div>
            ...
            <div className="flex gap-3">
              <button 
                onClick={prevSlide}
                className="w-12 h-12 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:text-red-600 hover:border-red-600 hover:shadow-xl transition-all active:scale-90"
...
const AppContent: React.FC = () => {
  const { user, isAdmin, isMember, isAuthenticated, logout, login, loginWithSSO } = useAuth();
  ...
  const handleSetBanners = async (newBanners: any[]) => {
    await db.setBanners(newBanners);
    setBanners(newBanners);
  };

  const handleSSOLogin = async (provider: 'GOOGLE' | 'FACEBOOK') => {
    setHandshakeData(prev => ({ ...prev, status: `OIDC_AUTH_${provider}` }));
    // In a real app, this triggers a redirect/popup. 
    // Here we simulate a successful return from the provider.
    const success = await loginWithSSO(
      provider === 'GOOGLE' ? 'vijay.fan@google.com' : 'tvk.member@facebook.com',
      provider === 'GOOGLE' ? 'Vijay Fan (SSO)' : 'TVK Member (SSO)',
      provider
    );
    if (success) {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'MANIFESTO': 
        return <ManifestoView onAddSuggestion={(text) => {}} />;
      case 'REGISTER':
          ...
          <div className="min-h-[80vh] flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
            <div className="bg-white dark:bg-gray-800 p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] shadow-2xl border border-gray-100 dark:border-gray-700 max-w-md w-full text-center">
              <div className="relative mb-6">
                ...
                <WhistleIcon className="w-24 h-24 mx-auto relative z-10" />
              </div>
              <h2 className="text-2xl md:text-3xl font-black mb-2 uppercase tracking-tight dark:text-white">Universal SSO</h2>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-8">Secure Access via TVK Identity Cloud</p>
              
              <div className="space-y-4">
                {/* Single Sign-On Providers */}
                <button 
                  onClick={() => handleSSOLogin('GOOGLE')}
                  className="w-full py-4 flex items-center justify-center gap-4 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl font-black uppercase tracking-widest hover:bg-gray-50 dark:hover:bg-gray-600 transition-all text-xs text-gray-700 dark:text-white"
                >
                ...
                <button 
                  ...
                  className="w-full py-4 flex items-center justify-center gap-4 bg-[#1877F2] text-white rounded-2xl font-black uppercase tracking-widest hover:opacity-90 transition-all text-xs"
                >
                ...
                <div className="flex items-center gap-4 py-4">
                  ...
                  <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">OR TVK DIRECT</span>
                  <div className="h-px flex-1 bg-gray-100 dark:bg-gray-700"></div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => { login('MEMBER'); navigateTo('DASHBOARD'); }} className="py-4 bg-gray-900 dark:bg-black text-white rounded-2xl font-black uppercase tracking-widest hover:bg-red-700 transition-all text-[10px]">Member Auth</button>
                  <button onClick={() => { login('ADMIN'); navigateTo('ADMIN'); }} className="py-4 border-2 border-gray-900 dark:border-white text-gray-900 dark:text-white rounded-2xl font-black uppercase tracking-widest hover:bg-gray-900 dark:hover:bg-white dark:hover:text-black transition-all text-[10px]">Admin Auth</button>
                </div>
                
                <div className="pt-6 border-t border-gray-100 dark:border-gray-700">
                  <button onClick={() => navigateTo('REGISTER')} className="w-full py-5 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-red-600/30 text-xs transform hover:scale-105 transition-transform">
      ...
      default: return (
        <>
          {/* Historic Icons Static Header (Visible on Home) */}
          ...
          {/* 5. Policy Vision - NOW AS CAROUSEL */}
    ...
    <div className={`min-h-screen font-poppins transition-colors duration-300 ${theme === 'dark' || theme === 'high-contrast' ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <div className="bg-gray-950 text-[8px] md:text-[9px] font-black text-white/40 py-1.5 uppercase tracking-[0.3em] border-b border-white/5 overflow-hidden">
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            <span className="text-green-500 animate-pulse font-bold">● POSTGRES_STORAGE_ACTIVE</span>
      ...
      <nav className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100 dark:border-gray-800 shadow-sm h-16 md:h-20" role="navigation">
        <div className="container mx-auto px-4 h-full flex justify-between items-center">
          {/* Enhanced Logo Section with Whistle Symbol */}
          <div className="flex items-center gap-4 cursor-pointer group" onClick={() => navigateTo('HOME')} aria-label="TVK Home">
            ...
            <div className="flex flex-col">
              <span className="text-xl md:text-2xl font-black text-gray-900 dark:text-white leading-none block tracking-tighter">
                {PARTY_NAME}
              </span>
              <span className="text-[9px] md:text-[11px] font-bold text-red-600 dark:text-red-500 uppercase tracking-[0.2em] mt-0.5 truncate max-w-[120px] md:max-w-none">
                {FULL_PARTY_NAME}
              </span>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-4">
                {isAdmin && (
                  <button 
                    ...
                    className={`font-black text-[10px] uppercase tracking-widest px-6 py-2 border-2 border-red-600 text-red-600 rounded-full hover:bg-red-600 hover:text-white transition-all shadow-lg ${currentView === 'ADMIN' ? 'bg-red-600 text-white' : ''}`}
                  >
                    Admin Panel
                  </button>
                )}
              ...
              <button onClick={() => navigateTo('LOGIN')} className="font-black text-[10px] uppercase tracking-widest px-6 py-2.5 bg-red-600 text-white rounded-full hover:bg-gray-900 transition-all shadow-xl shadow-red-600/30 transform hover:scale-105 active:scale-95">
                Login / Register
              </button>
            )}
            ...
            <div className="flex flex-col h-full pt-24 px-8 space-y-4 overflow-y-auto pb-10">
              ...
              <div className="pt-8 space-y-4">
                {isAuthenticated ? (
                  <div className="space-y-4">
                    {isAdmin && (
                      <button onClick={() => navigateTo('ADMIN')} className="w-full py-5 border-2 border-red-600 text-red-600 rounded-2xl font-black uppercase tracking-widest text-sm">Admin Command Center</button>
                    )}
                    ...
                    <button onClick={() => { logout(); navigateTo('HOME'); }} className="w-full text-center text-red-600 font-black uppercase tracking-widest text-xs">Logout Session</button>
                  </div>
                ) : (
                  <button onClick={() => navigateTo('LOGIN')} className="w-full py-5 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl shadow-red-600/30">Login / Register</button>
                )}
              </div>
